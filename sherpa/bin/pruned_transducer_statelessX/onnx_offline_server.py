...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="""The onnx model. You can use
          icefall/egs/librispeech/ASR/pruned_transducer_statelessX/export.py \
             --onnx=1
        to generate this model. All-in-one model.
        """,
    )
...
class OfflineServerOnnx:
    def __init__(
        ...
        Args:
          onnx_model_filename:
            Path to the onnx all-in-one model.
          bpe_model_filename:
        ...
        self.onnx_models = self._build_onnx_model(onnx_model_filename)
    ...
    def _build_onnx_model(
        self, onnx_model_filename: str
    ) -> List[List[onnxruntime.InferenceSession]]:
        ...
        encoder = self.get_onnx_session(
            self.model_onnx,
            ["encoder/x", "encoder/x_lens"],
            ["encoder/encoder_out", "encoder/encoder_out_lens"],
        )
        decoder = self.get_onnx_session(
            self.model_onnx,
            ["decoder/y"],
            ["decoder/decoder_out"],
        )
        joiner = self.get_onnx_session(
            self.model_onnx,
            [
                "joiner/encoder_out",
                "joiner/decoder_out",
        ...
        joiner_encoder_proj = self.get_onnx_session(
            self.model_onnx,
            ["joiner_encoder_proj/encoder_out"],
            ["joiner_encoder_proj/encoder_proj"],
        )
        joiner_decoder_proj = self.get_onnx_session(
            self.model_onnx,
        ...
        return [
            [encoder, decoder, joiner, joiner_encoder_proj, joiner_decoder_proj]
        ]
    ...
    async def feature_consumer_task(self):
        ...
        while True:
            ...
            self.counter = (self.counter + 1) % len(self.onnx_models)
            (
                encoder,
                decoder,
                joiner,
...
def main():
    ...
    offline_server = OfflineServerOnnx(
